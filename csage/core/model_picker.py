...
Saves config to ~/.csage/config.json.
API keys saved to ~/.csage/.env (chmod 600) via getpass (never echoed).
"""
...
logger = logging.getLogger("csage.picker")
...
def _save_api_key(provider: str, key: str):
    """Save API key to ~/.csage/.env with chmod 600."""
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
...
def run_picker(force: bool = False) -> LLMClient:
    from csage.utils.display import c, CYAN, BOLD, GREEN, YELLOW, GRAY, WHITE, ORANGE
    import builtins
    ...
    builtins.print(c("  ║       CSage — Model Setup             ║", CYAN))
    builtins.print(c("  ╚══════════════════════════════════════════╝", CYAN))
    builtins.print()
...
def _pick_local() -> dict:
    from csage.utils.display import c, CYAN, BOLD, GREEN, YELLOW, GRAY, ORANGE
    import builtins
...
def _ollama_flow(base_url: str) -> str:
    from csage.utils.display import c, CYAN, BOLD, GREEN, YELLOW, ORANGE, GRAY
    import builtins
...
def _pick_cloud(default: str | None = None) -> dict:
    ...
    if existing_key:
        ...
        if api_key:
            ...
            builtins.print(c("  ✓ Key saved to ~/.csage/.env (chmod 600)", GREEN))
        else:
...
def _ask(prompt: str = "") -> str:
    from csage.utils.display import c, BOLD, CYAN
    try:
