# Soteria Local Agent Playbook

This playbook is a practical guide to run Soteria with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Soteria starts directly.
...
Or let Soteria recommend the best local model for your goal:
...
```powershell
Get-Content .\.soteria-profile.json
```
...
- Keep any legacy workspace `.soteria-profile.json` local; new provider saves use the user config directory.
