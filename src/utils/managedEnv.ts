...
import { loadProfileFile, applyProfileEnvToProcessEnv } from './providerProfile.js'
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.soteria.json) is user-controlled. In CCD mode,
...
export function applyConfigEnvironmentVariables(): void {
  ...
  // Re-apply the legacy single-file profile LAST so it always wins over
  // stale settings.env (e.g. Ollama's OPENAI_MODEL lingering after switching
  // to Groq via /model). Without this, settings.env from a previous session
  // would overwrite the profile applied by cli.tsx.
  const persisted = loadProfileFile()
  if (persisted?.env) {
    applyProfileEnvToProcessEnv(process.env, persisted.env)
    // Ensure the CLAUDE_CODE_USE_* flag is set for OpenAI-shim providers.
    // The legacy profile env only has OPENAI_* vars, not the flag itself.
    const openAIShimProfiles = ['openai', 'custom', 'github', 'nvidia-nim', 'minimax', 'xiaomi-mimo', 'xai', 'codex']
    if (openAIShimProfiles.includes(persisted.profile)) {
      process.env.CLAUDE_CODE_USE_OPENAI = '1'
    } else if (persisted.profile === 'gemini') {
      process.env.CLAUDE_CODE_USE_GEMINI = '1'
    } else if (persisted.profile === 'mistral') {
