...
import {
  ...
  SOTERIA_PROJECT_INSTRUCTION_FILE,
} from './projectInstructions.js'

describe('projectInstructions', () => {
  test('prefers SOTERIA.md over AGENTS.md and CLAUDE.md', () => {
    const dir = '/repo'
    ...
    expect(filePath).toBe(join(dir, SOTERIA_PROJECT_INSTRUCTION_FILE))
  })

  test('prefers AGENTS.md over CLAUDE.md for root project instructions when SOTERIA.md is absent', () => {
    const dir = '/repo'
    const existingPaths = new Set([
      join(dir, PRIMARY_PROJECT_INSTRUCTION_FILE),
      join(dir, FALLBACK_PROJECT_INSTRUCTION_FILE),
  ...
  test('falls back to CLAUDE.md when both SOTERIA.md and AGENTS.md are absent', () => {
    const dir = '/repo'
  ...
  test('recognizes instruction filenames', () => {
    expect(isProjectInstructionFileName(SOTERIA_PROJECT_INSTRUCTION_FILE)).toBe(
      true,
    )
