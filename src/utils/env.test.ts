...
beforeEach(async () => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'soteria-env-test-'))
...
test('getGlobalClaudeFile: new install returns .soteria.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.soteria.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .soteria.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.soteria.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.soteria.json'))
})
