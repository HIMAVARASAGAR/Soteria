...
export const PROFILE_FILE_NAME = '.soteria-profile.json'
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  // Clear stale OpenAI-compat env from the previous session so the persisted
  // profile's values win. Without this, a prior Ollama session (which sets
  // OPENAI_BASE_URL=http://localhost:11434/v1) would silently override a saved
  // Groq/DeepSeek/OpenRouter profile applied via /model.
  delete baseEnv.OPENAI_BASE_URL
  delete baseEnv.OPENAI_API_BASE
  delete baseEnv.OPENAI_MODEL
  delete baseEnv.OPENAI_API_KEY
  delete baseEnv.OPENAI_API_FORMAT
  delete baseEnv.OPENAI_AUTH_HEADER
  delete baseEnv.OPENAI_AUTH_SCHEME
  delete baseEnv.OPENAI_AUTH_HEADER_VALUE
