...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/soteriaInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/soteria on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/soteria')
})

test('install command displays soteria.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'soteria.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both soteria and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.soteria'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.soteria', 'local'))
