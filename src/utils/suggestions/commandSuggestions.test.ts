...
describe('generateCommandSuggestions localization', () => {
  ...
  test('supports fuzzy/substring/partial matching of commands and aliases', () => {
    const commands = [
      promptCommand({
        name: 'commit',
        source: 'builtin',
        getDescription: () => 'Commit changes',
      }),
      promptCommand({
        name: 'output-style',
        source: 'builtin',
        getDescription: () => 'Change output style',
      }),
    ]

    // Substring match in the middle
    const results1 = generateCommandSuggestions('/mit', commands)
    expect(results1.length).toBe(1)
    expect(results1[0]?.displayText).toBe('/commit')

    // Substring match at the end
    const results2 = generateCommandSuggestions('/style', commands)
    expect(results2.length).toBe(1)
    expect(results2[0]?.displayText).toBe('/output-style')
  })

  test('ranks fuzzy name matches above description matches and handles typos', () => {
    const commands = [
      promptCommand({
        name: 'review',
        source: 'builtin',
        getDescription: () => 'Submit reviews for a pull request',
      }),
      promptCommand({
        name: 'theme',
        source: 'builtin',
        getDescription: () => 'Change color theme',
      }),
      promptCommand({
        name: 'commit',
        source: 'builtin',
        getDescription: () => 'Commit the staged changes',
      }),
    ]

    // Typo matching /thme -> /theme
    const resultsTheme = generateCommandSuggestions('/thme', commands)
    expect(resultsTheme[0]?.displayText).toBe('/theme')

    // Name match /mit -> /commit (ranks higher than /review which contains "Submit" in description)
    const resultsMit = generateCommandSuggestions('/mit', commands)
    expect(resultsMit[0]?.displayText).toBe('/commit')
  })

  test('does not return fuzzy-only description matches for command typos', () => {
    const commands = [
      promptCommand({
        name: 'theme',
        source: 'builtin',
      ...
      promptCommand({
        name: 'guide',
        source: 'builtin',
        getDescription: () => 'Document coding style conventions',
      }),
    ]

    expect(
      generateCommandSuggestions('/style', commands).map(
        item => item.displayText,
      ),
    ).toContain('/guide')

    expect(generateCommandSuggestions('/stlye', commands)).toEqual([])
  })
})
