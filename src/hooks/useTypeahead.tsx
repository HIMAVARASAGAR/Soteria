...
export function useTypeahead({
  ...
  // Stable refs to prevent updateSuggestions and fetchFileSuggestions recreation
  ...
  const suppressSuggestionsRef = useRef(suppressSuggestions);
  suppressSuggestionsRef.current = suppressSuggestions;

  const mcpResourcesRef = useRef(mcpResources);
  mcpResourcesRef.current = mcpResources;

  const agentsRef = useRef(agents);
  agentsRef.current = agents;

  const allCommandsMaxWidthRef = useRef(allCommandsMaxWidth);
  allCommandsMaxWidthRef.current = allCommandsMaxWidth;
  ...
  const fetchFileSuggestions = useCallback(async (searchToken: string, isAtSymbol = false): Promise<void> => {
    ...
    const combinedItems = await generateUnifiedSuggestions(searchToken, mcpResourcesRef.current, agentsRef.current, isAtSymbol);
  ...
  const updateSuggestions = useCallback(async (value: string, inputCursorOffset?: number): Promise<void> => {
    ...
    if (modeRef.current === 'prompt') {
      ...
      if (midInputCommand) {
        const match = getBestCommandMatch(midInputCommand.partialCommand, commandsRef.current);
        if (match) {
    ...
    if (modeRef.current === 'bash' && value.trim()) {
    ...
    const atMatch = modeRef.current !== 'bash' ? value.substring(0, effectiveCursorOffset).match(/(^|\s)@([\w-]*)$/) : null;
    if (atMatch) {
    ...
    if (modeRef.current === 'prompt' && isCommandInput(value) && effectiveCursorOffset > 0) {
    ...
    if (modeRef.current === 'prompt' && isCommandInput(value) && effectiveCursorOffset > 0 && !hasCommandWithArguments(isAtEndWithWhitespace, value)) {
      ...
      if (value.length > 1) {
        ...
        if (spaceIndex !== -1) {
          const exactMatch = commandsRef.current.find(cmd => getCommandName(cmd) === commandName);
      ...
      const commandItems = generateCommandSuggestions(value, commandsRef.current);
      setSuggestionsState(() => ({
        commandArgumentHint,
      ...
      if (commandItems.length > 0) {
        setMaxColumnWidth(allCommandsMaxWidthRef.current);
      }
      return;
    }
    ...
    if (hasAtSymbol && modeRef.current !== 'bash') {
    ...
    if (suggestionTypeRef.current === 'shell') {
      ...
      if (modeRef.current !== 'bash' || value !== inputSnapshot) {
        debouncedFetchFileSuggestions.cancel();
        clearSuggestions();
      }
    }
