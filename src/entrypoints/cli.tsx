...
// Soteria: polyfill globalThis.File for Node < 20.
...
// Soteria: disable experimental API betas by default.
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Soteria)`);
    return;
  }
  ...
  if (process.env.SOTERIA_DEBUG_MODEL) {
    console.error(`[CLI-DEBUG] startupEnv===process.env: ${startupEnv === process.env} startupEnv.OPENAI_MODEL=${startupEnv.OPENAI_MODEL}`)
  }
  if (startupEnv !== process.env) {
    ...
    if (process.env.SOTERIA_DEBUG_MODEL) {
      console.error(`[CLI-DEBUG] validationError=${startupProfileError} isDefault=${isDefaultStartupProviderEnv(startupEnv)}`)
    }
    if (startupProfileError && !isDefaultStartupProviderEnv(startupEnv)) {
      ...
      // Ensure CLAUDE_CODE_USE_OPENAI is set for OpenAI-shim providers so
      // that getAPIProvider() returns 'openai' (not 'firstParty').
      if (startupEnv.CLAUDE_CODE_USE_OPENAI === '1') {
  ...
  if (process.env.SOTERIA_DISABLE_EARLY_INPUT !== '1') {
    const {
