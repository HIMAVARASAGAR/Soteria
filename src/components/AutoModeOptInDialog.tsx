...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Soteria handle permission prompts automatically — Soteria checks each tool call for risky actions and prompt injection before executing. Actions Soteria identifies as safe are executed, while actions Soteria identifies as risky are blocked and Soteria may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Soteria can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
