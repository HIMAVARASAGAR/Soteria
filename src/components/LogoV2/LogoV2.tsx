...
import { BRAND_ACCENT_RGB, BRAND_NAME, BRAND_TAGLINE } from '../../constants/brand.js';
...
export function LogoV2() {
  ...
  const borderTitle = ` ${color("text", userTheme)("Soteria")} ${color("inactive", userTheme)(`v${version}`)} `;
  const compactBorderTitle = color("text", userTheme)(" Soteria ");
  if (layoutMode === "compact") {
    ...
    if ($[34] === Symbol.for("react.memo_cache_sentinel")) {
      t12 = <Text dimColor={true}>/model to switch models · /help for commands</Text>;
      $[34] = t12;
    } else {
      t12 = $[34];
    }
    ...
    return <><OffscreenFreeze><Box flexDirection="column" borderStyle="round" borderColor="inactive" borderText={t11} paddingX={2} paddingY={1} width={columns}><Text><Text color={BRAND_ACCENT_RGB} bold={true}>{BRAND_NAME}</Text><Text dimColor={true}> v{version}</Text></Text><Text bold={true}>{welcomeMessage}</Text>{t13}<Text dimColor={true}>{billingType}</Text><Text dimColor={true}>{agentName ? `@${agentName} · ${truncatedCwd}` : truncatedCwd}</Text>{t12}</Box></OffscreenFreeze>{t14}{t15}{t16}{t17}{t18}{t19}</>;
  }
  ...
  if ($[46] !== welcomeMessage_0) {
    t18 = <Box marginTop={1} flexDirection="column" alignItems="center"><Text><Text color={BRAND_ACCENT_RGB} bold={true}>{BRAND_NAME}</Text><Text dimColor={true}> v{version}</Text></Text><Text dimColor={true}>{BRAND_TAGLINE}</Text><Text color="inactive">────────────────</Text><Text bold={true}>{welcomeMessage_0}</Text></Box>;
    $[46] = welcomeMessage_0;
    $[47] = t18;
  } else {
    t18 = $[47];
  ...
  let t22;
  if ($[53] !== t20 || $[54] !== t21) {
    t22 = <Box flexDirection="column" alignItems="center"><Text dimColor={true}>────────────</Text>{t20}{t21}<Text><Text color="inactive">Quick</Text><Text dimColor={true}>   /model · /help · /doctor</Text></Text></Box>;
    $[53] = t20;
    $[54] = t21;
    $[55] = t22;
  } else {
  ...
  if ($[56] !== columns || $[57] !== t18 || $[58] !== t22) {
    t23 = <Box flexDirection="column" width="100%" justifyContent="center" alignItems="center" minHeight={9}><Box borderStyle="round" borderColor="inactive" paddingX={3} paddingY={1} width="100%" justifyContent="center" alignItems="center" minHeight={9}><Box flexDirection="column" alignItems="center" justifyContent="center">{t18}<Box marginY={1}>{t19}</Box>{t22}</Box></Box></Box>;
    $[56] = columns;
    $[57] = t18;
    $[58] = t22;
    $[59] = t23;
