...
// WelcomeHeader — clean bordered box shown above the message list on
// first render. Matches Claude Code's welcome screen style.
// Uses _c(3) to preserve react-compiler-runtime memo cache slot layout.
const WelcomeHeader = React.memo(function WelcomeHeader(t0: {
...
function WelcomeHeaderInner(): React.ReactNode {
  const version = MACRO.DISPLAY_VERSION ?? MACRO.VERSION;
  const providerInfo = useWelcomeProviderInfo();
...
const MessagesImpl = ({
  ...
  return <>
      {/* Logo */}
      {!hideLogo && !(renderRange && renderRange[0] > 0) && <WelcomeHeader agentDefinitions={agentDefinitions} />}
