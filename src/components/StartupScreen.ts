...
export function printStartupScreen(modelOverride?: string): void {
  ...
  const providerTypeText = p.isLocal ? 'local' : 'cloud'
  const version = MACRO.DISPLAY_VERSION ?? MACRO.VERSION
  const cwd = process.cwd()

  // Replace default home path with ~ for cleaner look
  ...
  const rightLines = [
    `${ansiRgb(...CREAM)}Soteria${RESET} ${DIM}${ansiRgb(...DIMCOL)}v${version}${RESET}`,
    `${ansiRgb(...CREAM)}${p.model}${RESET} ${DIM}${ansiRgb(...DIMCOL)}· ${p.name} (${providerTypeText})${RESET}`,
    `${DIM}${ansiRgb(...DIMCOL)}${displayCwd}${RESET}`
  ]

  const out: string[] = []
  out.push('') // empty leading line

  const total = MASCOT.length
  for (let i = 0; i < total; i++) {
    const mascotT = total > 1 ? i / (total - 1) : 0
    const coloredMascot = paintLine(MASCOT[i], GRAD, mascotT)
    const rightSide = rightLines[i] ?? ''
    out.push(`  ${coloredMascot}   ${rightSide}`)
  }

  out.push('') // empty trailing line
