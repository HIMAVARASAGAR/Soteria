...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'soteria-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.soteria', 'wiki', 'schema.md'),
    join('.soteria', 'wiki', 'index.md'),
    join('.soteria', 'wiki', 'log.md'),
    join('.soteria', 'wiki', 'pages', 'architecture.md'),
  ])
