...
const { ChatController, SoteriaChatViewProvider, SoteriaChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const SOTERIA_REPO_URL = 'https://github.com/HIMAVARASAGAR/Soteria';
const SOTERIA_SETUP_URL = 'https://github.com/HIMAVARASAGAR/Soteria/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.soteria-profile.json';
const SECRET_AZURE_API_KEY = 'soteria.azure.apiKey';
...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'Soteria Azure chat is enabled but endpoint or deployment is missing. Run "Soteria: Configure Azure / Foundry Chat" or set soteria.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'Soteria Azure chat is enabled but no API key is set. Use "Soteria: Set Azure / Foundry API Key" or soteria.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'Soteria — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('Soteria Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('Soteria Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('soteria');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'Soteria — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'Soteria — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'Soteria — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'Soteria Azure / Foundry chat saved. Launch Soteria from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('soteria');
  const launchCommand = configured.get('launchCommand', 'soteria');
  const terminalName = configured.get('terminalName', 'Soteria');
...
async function launchSoteria(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Soteria command not found: ${executable}. Install it with: npm install -g @himavarasagar/soteria@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(SOTERIA_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(SOTERIA_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Soteria because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Soteria wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Soteria project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Soteria commands quickly.</span>
            </button>
...
class SoteriaControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchSoteria({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'soteria.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  const provider = new SoteriaControlCenterProvider();
  ...
  const chatViewProvider = new SoteriaChatViewProvider(chatController);
  const chatPanelManager = new SoteriaChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) Soteria';
  statusBarItem.tooltip = 'Open Soteria Chat';
  statusBarItem.command = 'soteria.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Soteria';
        statusBarItem.tooltip = 'Soteria is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Soteria';
        statusBarItem.tooltip = 'Soteria connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Soteria';
        statusBarItem.tooltip = 'Open Soteria Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('soteria.start', async () => {
    await launchSoteria();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'soteria.startInWorkspaceRoot',
    async () => {
      await launchSoteria({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('soteria.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(SOTERIA_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'soteria.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(SOTERIA_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'soteria.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('soteria.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.soteria');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('soteria.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('soteria.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('soteria.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('soteria.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('soteria.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('soteria.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('soteria.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('soteria.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'soteria.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'soteria.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('soteria')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  SoteriaControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  SoteriaChatViewProvider,
  SoteriaChatPanelManager,
};
