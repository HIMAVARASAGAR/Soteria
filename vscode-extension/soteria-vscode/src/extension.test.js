...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'soteria',
    launchCommand: 'soteria --project-aware',
    terminalName: 'Soteria',
    shimEnabled: false,
    workspaceFolder: '/workspace/soteria/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/soteria/very/long/path/example-project',
    launchCwdLabel: '/workspace/soteria/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/soteria/very/long/path/example-project/.soteria-profile.json',
    workspaceProfilePath: '/workspace/soteria/very/long/path/example-project/.soteria-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Soteria wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/soteria\/very\/long\/path\/example-project"[^>]*>\/workspace\/soteria\/very\/long\/path\/example-project<\//,
  );
});
...
test('SoteriaControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { SoteriaControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/soteria/src/panels/control-center.js',
      workspacePath: '/workspace/soteria',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/soteria/src/panels',
      projectAwareCwdLabel: '/workspace/soteria/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/soteria',
      workspaceRootCwdLabel: '/workspace/soteria',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/soteria',
      activeFilePath: '/workspace/soteria/src/panels/control-center.js',
      workspacePath: '/workspace/soteria',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/soteria',
      projectAwareCwdLabel: '/workspace/soteria',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/soteria/src/panels',
      launchCwdLabel: '/workspace/soteria/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/soteria\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/soteria/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/soteria',
      launchCwdLabel: '/workspace/soteria',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/soteria/);
  assert.match(html, /Same workspace-root target as Launch Soteria because the relative command resolves from the workspace root · \/workspace\/soteria/);
});
