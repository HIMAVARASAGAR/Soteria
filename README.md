...
- **Continuous AI Pipeline:** When you execute an AI-suggested command, CodeSage automatically pipes the output back into the AI to find issues immediately without manual copying.
...
CodeSage acts as an orchestrator for several industry-standard security tools. For the most seamless experience, we recommend having these toolsets available on your system **before** running your first scan:
...
**Don't have them?** Don't worry. CodeSage includes an automated installation wizard that helps you fetch missing tools via `brew` (macOS), `apt` (Linux), or `winget` (Windows). If you don't use a package manager, CodeSage will provide direct download links for manual setup.

---

## Installation

CodeSage is designed to hook directly into your global system path for absolute convenience, allowing you to trigger audits from any directory.
...
To launch CodeSage against a target directory, IP, or URL simply type:

```bash
codesage https://example.com
```
...
CodeSage automatically handles configuration using an interactive setup wizard! 
...
3. **Saving State:** That key is natively encrypted and stored securely at `~/.codesage/.env` with strict `chmod 600` permissions. The model you selected will be saved to `~/.codesage/config.json`.

If you ever wish to modify these, enter `codesage --reset-model` or type `[model]` while inside an audit.
