/**
 * Soteria build script — bundles the TypeScript source into a single
...
result = await Bun.build({
  ...
  define: {
    ...
    // version separately in Soteria branding.
    'MACRO.VERSION': JSON.stringify('99.0.0'),
    'MACRO.DISPLAY_VERSION': JSON.stringify(version),
    'MACRO.BUILD_TIME': JSON.stringify(new Date().toISOString()),
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/HIMAVARASAGAR/Soteria/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/HIMAVARASAGAR/Soteria/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@himavarasagar/soteria'),
...
if (!result.success) {
  ...
  console.log(`✓ Built soteria v${version} → dist/cli.mjs`)
}
